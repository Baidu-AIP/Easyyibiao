...
from conf import __appname__
from conf import PY2
from conf import QT5
...
# from subWin.login.login_action import LoginWindow
# from subWin.show_dataset_list.show_dataset_list_action import SelectDataset
# from subWin.database.operation import SqLite
from export.export.export_widget_action import ExportWindow
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        exportToEasyData = action(
            self.tr("&导出到 EasyData"),
            self.exportToEasyData,
            shortcuts["export_to_easydata"],
            "to-data",
            # "labelme-Data",
            self.tr(u"Export labeled data to EasyData"),
            enabled=False
        )
        ...
        self.actions = utils.struct(
            ...
            exportToEasyData=exportToEasyData,
    ...
    def exportToEasyData(self):
        if not self.mayContinue():
            return

        if self.lastOpenDir and osp.isdir(self.lastOpenDir):
            ui = ExportWindow(self)
            ui.exec_()
    ...
    def importDirImages(self, dirpath, pattern=None, load=True):
        self.actions.openNextImg.setEnabled(True)
        self.actions.openPrevImg.setEnabled(True)
        self.actions.exportToEasyData.setEnabled(True)
